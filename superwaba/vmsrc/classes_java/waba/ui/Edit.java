...
public void onEvent(Event event)
   ...
   if (event.type == ControlEvent.WINDOW_CLOSED && event.target == calendar) // called from the keyboard and from the calendar
   {
   ...
   switch (event.type)
	  ...
	  case KeyEvent.KEY_PRESS:
		 ...
		 if ((ke.key == IKeys.KEYBOARD_ABC || ke.key == IKeys.KEYBOARD_123) && !(keyboard.isVisible() || calendar.isVisible()) ) // guich@102: pop's up the keyboard
		 {
			if (mode == DATE)
			{
