...
public class DataStream extends Stream
{
  ...
  public short readShort()
  {
   ...
   /** reads an string, converting from palm to java format.
   ...
   */
   public String readString()
   {
     ...
     byte []bytes = new byte[len]; // in java we can waste memory...
	  readBytes(bytes,0,len);
	  
	  return Convert.palm2javaString(bytes);
   }   
   /** reads an array of string. 
	   @returns a zero or more length array. null is never returned.
   */
  ...
  public int writeShort(int i)
  {
   ...
   public int writeString(String s)
   {
	  ...
	  {
		 byte[] c = waba.sys.Convert.java2palmString(s);
		 n += writeShort((short)c.length);
		 n += writeBytes(c,0,c.length);
	  }
	  else n += writeShort(0);      
	  return n;
   }   
