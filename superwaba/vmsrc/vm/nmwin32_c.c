...
#ifdef WINCE //ds@110 - catalog now emulated in waba.io.Catalog for win32
...
//	{ "waba/ui/MainWindow", NULL, 1 },
	{ "waba/ui/Window", NULL, 1 }, // ds@110
	{ "waba/ui/MainWindow", WindowDestroy, 1 }, // ds@110
...
//#define WOBJ_WindowHWnd(o) (objectPtr(o))[_winHookOffset + 0].refValue // ds@110
/* ds@110 - now only single window - so always point to it.  Should probably put
 * the window handle in MainWindow - but leave it in Window for now in case
 * I want to use seperate windows to implement popup window functionality using
 * the windows api instead of in the waba classes.
 */
...
static Var MainWinCreate(Var stack[])
	...
	wc = WOBJ_class(win);
	className = getUtfString(wc, wc->classNameIndex);
	n = className.len + 1;
	if (n > 40)
		n = 40;
...
static Var ImageSetPixels(Var stack[])
	...
	uchar *ppv;
	HDC hDC, hMemDC;
	HBITMAP hBitmap, hBlitBitmap, oldHBitmap;
	Var v;
...
static Var SocketCreate(Var stack[])
	...
	argp = 1;
	ioctlsocket(socketId, FIONBIO, &argp);

	connect(socketId, (struct sockaddr *)&sockAddr, sizeof(sockAddr));		
	...
	status = select(1, NULL, &fdSet, NULL, &tv);
	if (status <= 0){
		...
		case GR_COPYRECT:
			...
			if (dstSurf == srcSurf)
				...
				if (srcSurfaceType == SURF_WINDOW)
					...
					{
					HDC hMemDC;
					HBITMAP hBitmap, oldInputBitmap;					

					hBitmap = WOBJ_ImageHBitmap(srcSurf);
					if (hBitmap != NULL)
						{
						hMemDC = CreateCompatibleDC(hDC);
						oldInputBitmap = SelectObject(hMemDC, hBitmap);
						BitBlt(hDC, dstX, dstY, w, h, hMemDC, x, y, dwRop);
						SelectObject(hMemDC, oldInputBitmap);
						DeleteDC(hMemDC);
						}
					}
				}
...
static Var formatDouble(double doubleValue, int precision) // internal use only
{
   ...
   // if (FlpIsZero(doubleValue)) // ds@110
   if (doubleValue == 0)// ds@110
      buf[0] = '0';
   else
   {
      if (precision > 0)
      ...
      //StrIToA(&buf[len], (int32)i); // convert integer part // ds@110
      _itoa((int32)i, &buf[len], 10); // convert integer part // ds@110
      if (precision > 0)
      {
         len = xstrlen(buf);
         buf[len++] = '.';
...
//      return (double)StrAToI(str); //ds@110
		return atof(str);
   // convert the integer part
   *c++ = 0; // truncate the string
   //i = StrAToI(str); //ds@110
   i = atoi(str);
...
*/
static double stack2double(Var stack) // internal use only // ds@110
{
	double fDouble = 0;
    ...
    charArray = WOBJ_StringCharArrayObj(stack.obj);
	fDouble = wcstod((TCHAR *)WOBJ_arrayStart(charArray), &endPtr);
#else
	UtfString s;
	s = stringToUtf(stack.obj, STU_USE_STATIC | STU_NULL_TERMINATE);
	// find the position of '.'
...
{
	Var v;
	// v.floatValue = (float32)stack2double(stack[0]).d; //ds@110
	 v.floatValue = (float32)stack2double(stack[0]); //ds@110
	return v;
}
...
{
	//return formatDouble(stack2double(stack[0]).d,(int)stack[1].intValue); // ds@110
	return formatDouble(stack2double(stack[0]),(int)stack[1].intValue); // ds@110
}
...
{
	// FlpCompDouble fDouble1,fDouble2,fDouble3; // ds@110
	double fDouble1,fDouble2,fDouble3; // ds@110

	fDouble1 = stack2double(stack[0]);
	fDouble2 = stack2double(stack[1]);
	//fDouble3.fd = _d_add(fDouble1.fd,fDouble2.fd); // ds@110
	fDouble3 = fDouble1 + fDouble2; // ds@110
	//return formatDouble(fDouble3.d,PRECISION); // ds@110
	return formatDouble(fDouble3,PRECISION); // ds@110
}
...
static Var ConvertDivDoubles(Var stack[])
{
    ...
    fDouble1 = stack2double(stack[0]);
    fDouble2 = stack2double(stack[1]);
	// fDouble3.fd = _d_div(fDouble1.fd,fDouble2.fd); // ds@110
	fDouble3 = fDouble1 / fDouble2; // ds@110
	//return formatDouble(fDouble3.d,PRECISION); // ds@110
	return formatDouble(fDouble3,PRECISION); // ds@110
}
...
static Var ConvertMulDoubles(Var stack[])
{
    ...
    fDouble1 = stack2double(stack[0]);
    fDouble2 = stack2double(stack[1]);
	// fDouble3.fd = _d_mul(fDouble1.fd,fDouble2.fd); // ds@110
	fDouble3 = fDouble1 * fDouble2; // ds@110
	//return formatDouble(fDouble3.d,PRECISION); // ds@110
	return formatDouble(fDouble3,PRECISION); // ds@110
}
static Var ConvertSubDoubles(Var stack[])
{
    ...
    fDouble1 = stack2double(stack[0]);
    fDouble2 = stack2double(stack[1]);
	//fDouble3.fd = _d_sub(fDouble1.fd,fDouble2.fd); // ds@110
	fDouble3 = fDouble1 - fDouble2; // ds@110
	//return formatDouble(fDouble3.d,PRECISION); // ds@110
	return formatDouble(fDouble3,PRECISION); // ds@110
}
...
static Var CatalogAddRecordAt(Var stack[]) { Var v; v.intValue = -1; return v; } // ds@110
static Var CatalogReadObject(Var stack[]) { Var v; v.intValue = -1; return v; } // ds@110
static Var CatalogInspectRecord(Var stack[]) { Var v; v.intValue = -1; return v; } // ds@110
#else
...
static void _CatalogShuffleUp(WObject cat, int32 pos, int32 count) { // ds@110
	// shuffle down record keys in the database
	int32 i;
	HANDLE hDb = (HANDLE)WOBJ_CatalogHandle(cat);
...
static void _CatalogShuffleDown(WObject cat, int32 pos, int32 count) { // ds@110
	// shuffle down record keys in the database
	int32 i;
	HANDLE hDb = (HANDLE)WOBJ_CatalogHandle(cat);
...
static Var CatalogCreate(Var stack[])
	...
	dbOid = 0;
	hDb = CeOpenDatabase(&dbOid, nameBuf, CEVT_UI4, 0, NULL); // ds@110
...
static Var CatalogDeleteRecord(Var stack[])
	...
	if (CeDeleteRecord(hDb, recOid) != TRUE)
		return v;
	_CatalogShuffleDown(cat, pos, count);
	v.intValue = 1;
	return v;
	}
...
static Var CatalogAddRecordAt(Var stack[]) // ds@110
	{
	WObject cat;
	HANDLE hDb;
	CEOID recOid;
	...
	props[1].wFlags = 0;
	props[1].val.blob = blob;
	// get a space to insert the record
	_CatalogShuffleUp(cat, pos, count);
	// write the record
	recOid = CeWriteRecordProps(hDb, 0, 2, props);
	if (recOid == 0)
		{
		LocalFree(blob.lpb);
		// try & fix the database - this isn't very satisfactory.  Needs to be looked at.
		// Also same for DeleteRecord - if any database update in either of these two
		// functions fails it could corrupt the database.
		_CatalogShuffleDown(cat, pos, count);
		return v;
		}
	WOBJ_CatalogCurRecPos(cat) = pos;
...
static Var CatalogSetRecordPos(Var stack[])
	...
	if (!props)
		return v;
	/* ds@110 - it seems Windows CE can give you the properties in any order.
	   If you just add records & then retrieve them that worked fine.  However
	   the problem comes after deleteRecord - it does a CeWriteRecordProps with
	   only one prop, and that seems to change the order of the propeties.  You
	   could define the properties to get & the order to get them by passing
	   CEPROPIDs to CeReadRecordPos, however you can't then tell if there are more
	   than two properties - and so you can't tell if it's a valid record.
	...
	if (props[key].propid != CEVT_UI4 ||
		props[data].propid != CEVT_BLOB)
		{
		LocalFree(props);
		return v;
		}
	WOBJ_CatalogCurRecPos(cat) = pos;
	WOBJ_CatalogCurRecOid(cat) = (void *)recOid;
	WOBJ_CatalogCurRecPtr(cat) = (void *)props[data].val.blob.lpb; // ds@110
	WOBJ_CatalogCurRecAllocPtr(cat) = (void *)props;
	WOBJ_CatalogCurRecLen(cat) = props[data].val.blob.dwCount; // ds@110
	WOBJ_CatalogCurRecOffset(cat) = 0;
	WOBJ_CatalogCurRecModified(cat) = 0;
	v.intValue = 1;
	return v;
...
static Var CatalogReadObject(Var stack[]) { Var v; v.intValue = -1; return v; } // ds@110 - to implement
         ...
         if (!FIELD_is(field, ACCESS_TRANSIENT) && !FIELD_is(field, ACCESS_FINAL))
         {
            ...
            switch (fdesc.str[0])
            {
                  ...
                  if (!xstrncmp(stringArrayName,fdesc.str,fdesc.len)) // only String[] supported
                  {
                     ...
                     recBytes += 2;
                  	...
                  	if (pushObject(stringArray) == -1)
                  	{
                        ...
                        recBytes += 2;
                        strings[j] = createStringFromBuffer(recBytes,n);
                        recBytes += n;
               ...
               case 'L': // object
                  ...
                  {
                     debug("is string",2000);
                     n = read16(recBytes); // string length
                     recBytes += 2;
                     if (isStatic)
                        field->var.staticVar.obj = createStringFromBuffer(recBytes,n);
                     else
                        WOBJ_var(obj, field->var.varOffset).obj = createStringFromBuffer(recBytes,n);
                     recBytes += n;
                  }
                  break;
            }
...
static Var CatalogInspectRecord(Var stack[]) // ds@110
{
	WObject cat, byteArray;
	...
	size = props[data].val.blob.dwCount;
	if (count > size) count = size;

   // transfers the data
	xmemmove(bytes, props[data].val.blob.lpb, count);
	LocalFree(props);
...
static Var VmRandom(Var stack[]) // ds@110
{
	Var v;
	srand(stack[0].intValue); 
	v.intValue = (long)rand() << 16L | (long)rand();
	return v;
}
...
static Var VmGetRomVersion(Var stack[]) // guich // ds@110
{
   Var v;
   ...
   v.intValue = (osvi.dwMajorVersion * 100) + osvi.dwMinorVersion; 
   return v;
}
static Var VmGetFreeClassHeap(Var stack[]) // guich
{
   Var v;
   gc();
...
static Var VmGetFreeObjectHeap(Var stack[]) // guich 
{
   Var v;
...
static Var VmClipboardCopy(Var stack[]) // ds@110
{
	Var v;
	HANDLE hClipData;                           /* handle to clip data  */ 
	LPTSTR lpClipData;                           /* pointer to clip data */ 
    WObject charArray;
    uint32 i, len;
    uint16 *chars;
	
	v.obj = 0;
    charArray = WOBJ_StringCharArrayObj(stack[0].obj);
    if (charArray == 0) {
        return v;
	}
    len = WOBJ_arrayLen(charArray);
#ifdef WINCE
	if (!(hClipData = LocalAlloc(LPTR, (len + 1) * sizeof(TCHAR)))) { 
		return v;
	} 
	lpClipData = (LPTSTR) hClipData;
#else
	if (!(hClipData = GlobalAlloc(GMEM_DDESHARE | GMEM_MOVEABLE , (len + 1) * sizeof(TCHAR)))) { 
		return v;
	} 
    ...
    chars = (uint16 *)WOBJ_arrayStart(charArray);
    for (i = 0; i < len; i++)
		// lets keep the clipboard data in the right format for the system
#ifdef WINCE
        lpClipData[i] = chars[i];
#else
		lpClipData[i] = (char) chars[i];
#endif
    lpClipData[i] = 0;
	...
	*/ 
	if (OpenClipboard((HWND)(WOBJ_WindowHWnd(mainWinObj)))) { 
		EmptyClipboard();
#ifdef WINCE
...
static Var VmClipboardPaste(Var stack[]) // ds@110
{
	Var v;
	HANDLE hClipData;                           /* handle to clip data  */ 
	LPTSTR lpClipData;                           /* pointer to clip data */ 
...
#ifdef WINCE
		lpClipData = (LPTSTR) hClipData;
//		v.obj = createString(lpClipData);
		v.obj = createStringFromUnicode(lpClipData, _tcslen(lpClipData));
#else
		if (!(lpClipData = GlobalLock(hClipData))) { 
			CloseClipboard(); 
			v.obj = createString("");
...
static void VmGetSettings() // guich // ds@110
{
#define cchData 256
   int date;
   char dateSep;
   ...
   // get the preferences
   // date format
	if (!GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_IDATE, data, cchData)) {
   		date = 2; //default
	} else {
		date = data[0] - TEXT('0') + 1;
	}
	
	// date seperator
	if (!GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SDATE, data, cchData)) {
		dateSep = '/'; 
	} else {
		dateSep = (char) data[0];
	}
	...
	// start of week
	if (!GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_IFIRSTDAYOFWEEK, data, cchData)) {
		weekStart = 0;
	} else {
		if (data[0] == TEXT('7')) {
			weekStart = 0;
		} else {
			weekStart = data[0] - TEXT('0') + 1;
		}
	}

	// time seperator
	if (!GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_STIME, data, cchData)) {
		timeSep = ':'; 
	} else {
		timeSep = (char) data[0];
	}
	...
	// 24 hour format
	if (!GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_ITIME, data, cchData)) {
		time24h = 0;
	} else {
		time24h = data[0] - TEXT('0');
	}

	// thousands seperator
	if (!GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_STHOUSAND, data, cchData)) {
		ts = ',';
	} else {
		ts = (char) data[0];
	}
	...
	// decimal seperator
	if (!GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SDECIMAL, data, cchData)) {
		ds = '.';
	} else {
		ds = (char) data[0];
	}
   ...
   prefObj = createObject(prefClass);
   ...
   if (method != NULL)
      executeMethod(prefClass, method, params, 9);

}

static int stateX=170,stateY=170;
/*static void showCommand(Boolean show) // guich@102
{
...
/*   Boolean capsLockP;
   ...
   //GsiSetLocation(stateX,stateY+2); ds@110
//   GrfGetState (&capsLockP,&numLockP,&tempShiftP,&autoShiftedP);
