...
static void debug(char *msg, int wait)
{
	RectangleType rect;
	if (debugY < traceMinY) debugY = traceMinY;
	rect.topLeft.x = 0;
	rect.topLeft.y = debugY;
	rect.extent.x = 160;
	rect.extent.y = 10;
	WinEraseRectangle(&rect,0);
   WinDrawChars(msg, xstrlen(msg), 0, debugY);
   debugY += 10;
   if (debugY >= traceMaxY) debugY = traceMinY;
   if (wait) SysTaskDelay(millisToTicks(wait));
}
static void drawErrorWin()
   ...
   if (traceOn) // guich: if trace is on, make a delay so the programmer can see the stack trace
   {
      SndPlaySystemSound(sndInfo);
      debug("Fatal Error. 20 seconds to dismiss...",20000);
   }
   ...
   SysTaskDelay(millisToTicks(5000)); // guich: added this because sometimes a click is buffered and there is no time to see the screen.
...
/*			{
			   ...
			   cc = (capsLockP << 1) | (tempShiftP << 2);
			   buff[i++] = '0' + chr / 100;
			   buff[i++] = '0' + chr / 10 % 10;
			   buff[i++] = '0' + chr % 10;
			   buff[i++] = '.';
			   buff[i++] = '0' + eventP->data.keyDown.modifiers / 100;
			   buff[i++] = '0' + eventP->data.keyDown.modifiers / 10 % 10;
			   buff[i++] = '0' + eventP->data.keyDown.modifiers % 10;
			   buff[i++] = '.';
			   buff[i++] = '0' + cc / 100;
			   buff[i++] = '0' + cc / 10 % 10;
			   buff[i++] = '0' + cc % 10;
...
//				case :             key = 75013; break; // DELETE
				...
				case commandChr:   key = 75015; isCommand = !isCommand; showCommand(isCommand); break; // COMMAND
				// lines below added by guich
		   ...
		   if (key != 75015 && isCommand) // guich: different key and command?
		   {		      
		      isCommand = false;
		      showCommand(false);
		   }
			...
			if (method != NULL)
				...
				params[5].intValue = 0; // modifiers - guich: why dont use the modifiers? now i use it!
				
            GrfGetState(&capsLockP,&numLockP,&tempShiftP,&autoShiftedP);
				if (eventP->data.keyDown.modifiers & controlKeyMask || chr == commandChr) params[5].intValue |= (1 << 1);
				if (eventP->data.keyDown.modifiers & shiftKeyMask || tempShiftP || autoShiftedP)   params[5].intValue |= (1 << 2);
				if (eventP->data.keyDown.modifiers & capsLockMask || capsLockP)   params[5].intValue |= (1 << 3);
				if (eventP->data.keyDown.modifiers & commandKeyMask) params[5].intValue |= (1 << 4);
				if (eventP->data.keyDown.modifiers & optionKeyMask)  params[5].intValue |= (1 << 5);
				params[6].intValue = 0; // timeStamp
				executeMethod(vclass, method, params, 7);
				}
			break;
...
#ifdef SECURE_OBJECT_HEAP
static DmOpenRef objectHeapDmRef = 0;
static VoidHand objectHeapRecH = 0;
static VoidPtr objectHeapRecP = 0;

static void delObjectHeapDb()
	{
	UInt cardNo;
	LocalID dbID;
...
static WObject startApp(int runApp)
	...
	while (cmdLine[i] == '/')
		...
		{
			objectHeapSize = value>64000?64000:value; // guich: 64000 maximum
			if (objectHeapSize < 40000) objectHeapSize = 40000;
		}
		else if (c == 's')
			vmStackSize = value;
		else if (c == 't')
...
static char *int2str(int ii)
{
   static char buff[6];
   int i =0;
   buff[i++] = '0' + (ii / 10000 % 10);
   buff[i++] = '0' + (ii / 1000  % 10);
   buff[i++] = '0' + (ii / 100   % 10);
   buff[i++] = '0' + (ii / 10    % 10);
   buff[i++] = '0' + (ii / 1     % 10);
...
extern int interceptSystemKey; // guich
...
*/      
      // NOTE: popping up the keyboard without a form will cause PalmOS
      // to reset 
      // modified by guich: let the user intercept these keys (and do what they want!)
      if (!(event.eType == keyDownEvent && (
         (event.data.keyDown.chr == keyboardAlphaChr) || // inconditional intercept
         (event.data.keyDown.chr == keyboardNumericChr) || // inconditional intercept
         (event.data.keyDown.chr == keyboardChr) ||  // inconditional intercept
         (interceptSystemKey && 
           ((event.data.keyDown.chr == hard1Chr)    || // Conditional intercept
            (event.data.keyDown.chr == hard2Chr)    || // Conditional intercept
            (event.data.keyDown.chr == hard3Chr)    || // Conditional intercept
            (event.data.keyDown.chr == hard4Chr)    || // Conditional intercept
            (event.data.keyDown.chr == calcChr)     || // Conditional intercept
            (event.data.keyDown.chr == findChr))) // Conditional intercept
         )))
      {
