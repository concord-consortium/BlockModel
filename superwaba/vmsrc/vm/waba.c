...
//#define WIN32 1
//#define PALMOS 1 // guich@120 - back to palm os
#define WINCE 1
 
#if defined(NO_PLATFORM_DEFINED)
To compile, you need to define one of the platforms above.
#endif
#ifdef WIN32 //ds@110 - catalog now emulated in waba.io.Catalog for win32
#define WIN32_NOCATALOG
#endif
#if defined(WINCE)
#define WIN32 1
#endif
...
#define ERR_CantAllocateObjectMemory 22
...
#define FIELD_is(f,acc) ((FIELD_accessFlags(field) & acc) > 0)
...
static void executeMethod(WClass *wclass, WClassMethod *method,Var params[], uint32 numParams);
static void initHashTables(); // guich
static uchar utf2char(int32 utf0, int32 utf1, int32 utf2); // guich
static void debug(char *msg, int wait); // guich: to debug the vm
static void VmGetSettings(); // guich
static void showCommand(int show); // guich  // ds@110 change Boolean to int to run on Win32
...
// variables added by guich
static int traceOn;
static int32 traceDelay;
/////
...
//////////////////////////////////////////////////////////////////////////////////////
// guich@102 - implements a hash table, whoose key is a int32 and value can be anything. ps: HASHSIZE is fix. increase it if necessary.
struct htElem 
{
    uint32 key;              
    void *value;            
...
#define HASHSIZE 291  // guich: this strange value is heuristic: ive dont a small program in c with both tables to get the best cost/benefit value

static struct htElem *htMethods[HASHSIZE];         /* The storage array for the methods */
static struct htElem *htUTF[HASHSIZE];         /* The storage array for the utf table */

static unsigned htHash(uint32 key)
{
   return (unsigned)(key % HASHSIZE);
}
/* return the htElem found, else NULL if key doesn't exist */
static struct htElem *htGet(struct htElem **hashtab, uint32 key)
{
   struct htElem *np;
   for (np = hashtab[htHash(key)]; np; np = np->next) 
      if (np->key == key)
         return np;
   return NULL;
}
/* return struct containing value or NULL if unable to store */
static struct htElem *htPut(struct htElem **hashtab, uint32 key, void *value)
{
   struct htElem *np;
   unsigned hashval;
   if ((np = htGet(hashtab,key)) == NULL) // duplicated keys not allowed
   {
      np = (struct htElem *) xmalloc (sizeof(struct htElem));
      if (np != NULL)
      {
          hashval = htHash(key);
...
/* free up the hash tree (garbage collect), also free up any xstrdup()'d text or mandata structures */
static void htFree(struct htElem **hashtab)
{
   int i;
...
static void VmInit(uint32 vmStackSizeInBytes, uint32 nmStackSizeInBytes,
    ...
    if (status != 0)
        {
        VmQuickError(ERR_CantAllocateObjectMemory);
        return;
        }
    vmInitialized = 1;
    }

// copies one or two UtfStrings (by concatenation) into a character
...
static WObject VmStartApp(char *className)
    ...
    if (!compatible(wclass, mainWinClass))
        return 0;
    VmGetSettings();     // guich: get the preferences before anything! the user may use them in the onStart method
    // 
#ifdef PALMOS // // ds@110 - functions not available on Win32
    GsiInitialize();
    GsiSetLocation(170,170); // guich: move to an invisible position
    GsiEnable(true);
#endif // // ds@110 
...
static void VmStopApp(WObject mainWinObj)
    ...
    if (!vmInitialized || mainWinObj == 0)
        return;
    // call onExit() - removed from here by guich and placed at postStopEvent
...
// modified by guich to correct handle international char support
static WObject createStringFromUtf(UtfString s)
    {
    WObject obj, charArrayObj;
    uint16 *charStart;
    uint32 i,j; 
//    int len = s.len; // ds@110 lets's keep em all the same type - avoids compiler warnings
//    int newLen = len;// ds@110 
    uint32 len = s.len;// ds@110 
    uint32 newLen = len;// ds@110 
    // finds if the string has international chars
    for (i = 0; i < len; i++)
       if (s.str[i] < 0) 
          if ((uchar)s.str[i] >> 4 == 0xC) // accentuation - 2 bytes length
          {
             newLen--;    
             i++;
          }
          ...
          {
             newLen -= 2; // intl - 3 bytes length
             i += 2;
          }
    // create and fill char array
    charArrayObj = createArrayObject(5, newLen);
    if (!charArrayObj)
        return 0;
    if (pushObject(charArrayObj) == -1)
        return 0;
    charStart = (uint16 *)WOBJ_arrayStart(charArrayObj);
    for (i = 0,j=0; i < s.len; j++)
       if (s.str[i] >= 0)
          charStart[j] =(uint16)s.str[i++];
       else
       if ((uchar)s.str[i] >> 4 == 0xC) // accentuation - 2 bytes length
       {
          charStart[j] = utf2char((int32)(uchar)s.str[i],(int32)(uchar)s.str[i+1],0);
          i += 2;
       } 
       else
       if ((uchar)s.str[i] >> 4 == 0xE) // intl - 3 bytes length
       {
          charStart[j] = utf2char((int32)(uchar)s.str[i],(int32)(uchar)s.str[i+1],(int32)(uchar)s.str[i+2]);
          i += 3;
       }   
...
*/
// guich@1.1.2
//static WObject createStringFromBuffer(uchar *ptr, int len)  // ds@110 lets's keep em all the same type - avoids compiler warnings
static WObject createStringFromBuffer(uchar *ptr, uint32 len) 
{
    WObject obj, charArrayObj;
    uint16 *charStart;
    uint32 i;
...
// dont exist in the table: 129 141 142 143 144 157 158
utfchar utfTable[] =
{
    {0xE282AC,(uchar)128},
    {0xE2809A,(uchar)130},
...
// guich@102 - international support
static uchar utf2char(int32 utf0, int32 utf1, int32 utf2)
{
   uint32 key;
   struct htElem *elem;
   //static String dummy = "­@âÄãÉ àöäÞÜÎ@@@@ÔÕÒÓ¥ÐÑ÷ªßÝÏ@@ÙÊÁ¢£Û´½¤¬©»ÇÂð¨ø¡ÆÃÅ«µ¦áüÿ¼ÈùÚ×ÀËçåÌ€®‚éƒæèíêëìõ„ñîïÍ…û¯ôòó†ú¸§ˆ‡‰‹ŠŒ¾Ž‘“’”•þ–˜—™›šÖ¿œžŸÿ¹Ø";
   if (utf2 == 0)
      key = (utf0 << 8) | utf1;
   else
      key = (utf0 << 16) | (utf1 << 8) | utf2;
   elem = htGet(htUTF,key);
   if (elem == NULL) return '@';
   return (uchar)(*(uchar *)elem->value);
}
...
// guich: this method is being called from VmGetFreeObjectHeap
static uint32 getUnusedMem()
    {
...
#ifndef SECURE_OBJECT_HEAP
    heap.mem = (uchar *)xmalloc(heap.memSize);
#else
    delObjectHeapDb();
	// create a db (ObjHeap) for the object heap
	if (DmCreateDatabase(0, "ObjHeap", 'WABA', 'Ohep', false) == 0)
		{
		objectHeapDmRef = DmOpenDatabaseByTypeCreator('Ohep', 'WABA',
			dmModeReadWrite);
		if (objectHeapDmRef != 0)
			{
			UInt i;

			i = 0;
			objectHeapRecH = DmNewRecord(objectHeapDmRef, &i, heap.memSize);
			if (objectHeapRecH != 0)
			{
				objectHeapRecP = MemHandleLock(objectHeapRecH);
		    } else 
		    {
    		   return -1;
		    }
...
static void freeObjectHeap()
    ...
    if (heap.mem)
        xfree(heap.mem);
#else
	// close the object heap db
	if (objectHeapRecP != 0)
		MemHandleUnlock(objectHeapRecH);
	if (objectHeapRecH != 0)
		DmReleaseRecord(objectHeapDmRef, 0, false);
	if (objectHeapDmRef != 0)
		DmCloseDatabase(objectHeapDmRef);
    delObjectHeapDb();
#endif
    }
...
// guich: since i introduced a hash table, this table dont has anymore to stay sorted. :-))
NativeMethod nativeMethods[] =
    {
#ifndef WIN32_NOCATALOG //ds@110 - catalog now emulated in waba.io.Catalog for win32
    ...
    // waba/io/Catalog_addRecord_(II)I - guich
    { 1661938958, CatalogAddRecordAt },
    ...
    // waba/io/Catalog_inspectRecord_([BI)I  - guich
    { 1661973843, CatalogInspectRecord },
    // waba/io/Catalog_isOpen_()Z
    { 1661978185, CatalogIsOpen },
    // waba/io/Catalog_delete_()Z
    { 1661978505, CatalogDelete },
    // waba/io/Catalog_readObject_(Ljava/lang/Object;IB)Z   - guich
   // { 1661987425, CatalogReadObject },
#endif //ds@110 - catalog now emulated in waba.io.Catalog for win32
    // waba/sys/Vm_exec_(Ljava/lang/String;Ljava/lang/String;IZ)I
    { 113969325, VmExec },
    // waba/sys/Vm_clipboardCopy_(Ljava/lang/String;)V    - guich
    { 113976482, VmClipboardCopy },
    // waba/sys/Vm_clipboardPaste_()Ljava/lang/String;    - guich
    { 113977250, VmClipboardPaste },  
    // waba/sys/Vm_setTrace_(ZIII)V      - guich
    { 113984975, VmSetTrace },
    // waba/sys/Vm_getTimeStamp_()I
    { 113990543, VmGetTimeStamp },
    // waba/sys/Vm_debug_(Ljava/lang/String;I)V       - guich
    { 113993179, VmDebug },
    // waba/sys/Vm_getRomVersion_()I   - guich
    { 113998736, VmGetRomVersion },
    // waba/sys/Vm_copyArray_(Ljava/lang/Object;ILjava/lang/Object;II)Z
    { 114004019, copyArray },
    // waba/sys/Vm_getFreeClassHeap_()I - guich
    { 114013203, VmGetFreeClassHeap },
    // waba/sys/Vm_sleep_(I)V
    { 114016841, VmSleep },
    // waba/sys/Vm_getFreeObjectHeap_()I - guich
    { 114019412, VmGetFreeObjectHeap },
    ...
    // waba/sys/Vm_random_(I)I
    { 114022666, VmRandom },
    // waba/sys/Vm_getPlatform_()Ljava/lang/String;
    { 114023839, VmGetPlatform },
    // waba/sys/Vm_isColor_()Z
    { 114024842, VmIsColor },
    // waba/sys/Vm_setSystemKeysUse_(Z)V     - added by guich
    { 114026196, VmSetSystemKeys },
    ...
    // waba/ui/Window_nativeSetStatePosition_(II)V      - guich
    { 1406046363, WindowSetStatePosition },
    ...
    // waba/sys/Convert_toString_(Ljava/lang/String;I)Ljava/lang/String;   - guich
    { 2387627311, ConvertStringToDoubleStr },
    // waba/sys/Convert_toInt_(Ljava/lang/String;)I
    { 2387628570, ConvertStringToInt },
    // waba/sys/Convert_add_(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;  - guich
    { 2387629755, ConvertAddDoubles },
    // waba/sys/Convert_div_(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;  - guich
    { 2387631419, ConvertDivDoubles },
    // waba/sys/Convert_sub_(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;  - guich
    { 2387631867, ConvertSubDoubles },
    // waba/sys/Convert_mul_(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;  - guich
    { 2387632123, ConvertMulDoubles },
    // waba/sys/Convert_toIntBitwise_(F)I
    { 2387636560, ConvertFloatToIntBitwise },
    // waba/sys/Convert_toFloat_(Ljava/lang/String;)F    - guich
    { 2387641372, ConvertStringToFloat },
    ...
    // waba/sys/Convert_toString_(FI)Ljava/lang/String; - guich
    { 2387654302, ConvertFloatToStringFormatted },
...
// guich - build the hashtables
static void initHashTables()
{
   int n,i;
   n = sizeof(nativeMethods) / sizeof(NativeMethod);
   for (i = 0; i < n; i++)
      htPut(htMethods, nativeMethods[i].hash, nativeMethods[i].func);
   n = sizeof(utfTable) / sizeof(utfchar);
   for (i = 0; i < n; i++)
      htPut(htUTF, utfTable[i].utf, &utfTable[i].ch);
}
...
#ifdef USE_HASHTABLE    
    // guich: now its a hashtable!
    elem = htGet(htMethods,hash);
    if (elem != NULL)
       return (NativeFunc)elem->value;
#else
...
static char traceBuf[50];
static int32 countTrace = 0;
static void trace(WClass *iclass, WClassMethod *imethod)
{
   int j=0,l;
   char *c;
   UtfString className = getUtfString(iclass, iclass->classNameIndex);
   UtfString methodName = (imethod==NULL)?createUtfString("new "): getUtfString(iclass, METH_nameIndex(imethod));
   int cc = countTrace++ % 100;
   
   // finds the '/' of the class. ignores before it
   l = className.len;
   c = &className.str[l-1];
   while (*c != '/' && l >0) {l--; c--;}
   if (*c == '/') c++;
   // concats the strings
   traceBuf[j++] = (char)((cc / 10) + '0');
   traceBuf[j++] = (char)((cc % 10) + '0');
   traceBuf[j++] = ':';
   traceBuf[j++] = ' ';
   for (; *c; j++)
...
step:
    switch (*pc)
        ...
        case OP_invokespecial:
            ...
            if (traceOn) // guich - provide some debug facility
               trace(iclass,imethod);
