...
public class PdbFile extends WarpFile
{
  ...
  public String getName()
  {
    if (name==null)
    {
      ...
      if (name.length()>30)
      {
        System.out.println("ERROR: warp file name must be less than 31 characters");
        System.exit(-1);
      }
    }
    return name;
  ...
  protected void listFiles(DataInputStream dis,int numFiles) throws IOException
  {
    ...
    for(int i=0;i<numFiles;i++)
    {
      ...
      String name=Utils.readString(dis,(int)pathLen);
      int size;
      if (i<numFiles-1)
        size=offsets[i+1]-offsets[i]-pathLen-2;
      else
        size=(int)warpFile.length()-offsets[i]-pathLen-2;
